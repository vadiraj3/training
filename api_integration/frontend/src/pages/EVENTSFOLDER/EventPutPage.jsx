...
const EventPutPage = () => {
    ...
    const fatch = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3000/api/records/events/${params.id}`,
        );
        const { title, date, location, attendees } = response.data;
        setTitle(title);
        setDate(date);
  ...
  const handelSubmit = async () => {
    ...
    console.log(title, date, location, attendees);
